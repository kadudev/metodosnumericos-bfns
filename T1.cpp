...
#include <cmath> //para uso da matemática no cód
...
#include <fstream> //para salvar erros em arquivo
 ...
 double BissecaoMetodo(double xl, double xu, double eps, vector<double>& ErrosBissecao){
    ...
    if(func(xl)*func(xu) >=0){
        ...
        cout<<"Intervalo inválido. f(xl) * f(xu) deve ser menor que zero para haver troca de sinais."<<endl;
        return NAN; 
    } 
...
double FalsaPosicaoMetodo(double xl, double xu, double eps, vector<double>& ErrosFalsaPosicao){
    if(func(xl)*func(xu) >=0){
        ...
        cout<<"Intervalo inválido. f(xl) * f(xu) devem ser menor que zero para haver troca de sinais."<<endl;
        return NAN; 
    } 
